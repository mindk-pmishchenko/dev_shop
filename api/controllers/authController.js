...
class UserController extends baseController {
    ...
    static forgotPassword = catchAsync(async (req, res, next, err) => {
        ...
        const { token, reset_token, reset_token_expiration } = await this.resetPasswordTokenGen(); //returns { token, reset_token,reset_token_expiration}
        ...
        const resetUrl = `${req.protocol}://${req.get('host')}/api/users/reset_password/${token}`;
        ...
        try {
            ...
            return next(new AppError("Email with reseting password token wasn't sent"), 500);
        }
    });
    ...
    static resetPassword = catchAsync(async (req, res, next, err) => {
        ...
        if (!password || !passwordConfirmation) {
            return next(new AppError("Password or passwordConfirmation can't be blank"));
        }
        //must be the same
        if (password !== passwordConfirmation) {
            return next(new AppError('Password and passwordConfirmation feilds must be the same'));
        }
        ...
        if (new Date(currentUser[0].reset_token_expiration).getTime() < Date.now()) {
            return next(
                new AppError(
    ...
    static googleLogin = catchAsync(async (req, res, next, err) => {
        let temporaryPassword = Math.random().toString(36);
        const data = await new User(req).googleLogin(temporaryPassword);
        console.log('data', data);
        req.body.login = data.email;
    ...
    static protect = catchAsync(async (req, res, next) => {
        ...
        const decodeToken = await promisify(jwt.verify)(token, process.env.JWT_SEC_KEY);
        if (!decodeToken) {
        ...
        currentDbUser.forEach(el => {
            if (el.token === token) {
                if (!el.user_active_status) {
                    return next(
                        new AppError('Your account was disabled. Please contact to administrator')
                    );
                }
                accessGranted = true;
            }
        ...
        if (!accessGranted || decodeToken.exp < Date.now() / 1000) {
            return next(new AppError('Your credentials are outdated.Please login again'));
        } else {
            //req.user init
    ...
    static restictTo = (...roles) => {
        return (req, res, next) => {
            if (!roles.includes(req.user.role)) {
                return next(new AppError('You have no permission for doing this action', 403));
            }
    ...
    static changePersonalPassword = catchAsync(async (req, res, next) => {
        ...
        const updatedUser = await new Pure(req).changePersonalPassword(filteredBody);
