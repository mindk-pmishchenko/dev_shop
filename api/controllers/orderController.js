...
class OrderController extends baseController {
    static changeItemQuantity = catchAsync(async (req, res, next) => {
        ...
        if (!actions.includes(action)) {
            return next(new AppError('Wrong action type', 400));
        }

        const dbResponse = await new Pure(req).changeItemQuantity();
