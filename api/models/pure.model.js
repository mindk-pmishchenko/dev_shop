...
class Pure extends BaseModel {
    ...
    getAllProducts() {
        ...
        const order = allowedOrder.includes(this.query.order) ? this.query.order : 'asc';
        /// order BY ["id","title", "quantity","price","available","valid_until"]
        const allowedOrderBy = ['id', 'title', 'quantity', 'price', 'available', 'valid_until'];
        const orderBy = allowedOrderBy.includes(this.query.orderby) ? this.query.orderby : 'id';
        //console.log(order);
                        ...
                        if (el === 'price') {
                            ...
                            if (el === 'category_id') {
                                ...
                                if (this.checkIntValue(this.query[el], 'category_id', false)) {
                                    ...
                                    builder.where(el, this.query[el]);
                                } else if (!isNaN(this.query[el]) || rexExp.test(this.query[el])) {
    ...
    checkout = async () => {
        ...
        const allowedFields = ['delivery_type', 'delivery_address', 'contact_phone'];
            ...
            try {
                ...
                const checkout = await trx
                    ...
                    .then(async orderData => {
                                    ...
                                    .then(async shopsProductAvailableQuantity => {
                                        ...
                                        for (
                                            ...
                                            if (
                                                ...
                                                quantityForUpdate[
                                                    shopsProductAvailableQuantity[i].id
                                                ] =
                                                    shopsProductAvailableQuantity[i].quantity -
                                                    clientsOrder[i].quantity;
                                            }
                        ...
                        if (availableDelivery.indexOf(filteredBody.delivery_type * 1) === -1) {
    ...
    updateOrder = async () => {
        //allow just allowed fields from body
        const allowedFields = ['active', 'delivery_type', 'delivery_address', 'contact_phone'];
